...
// Get PAR from SAR and DAR.
static AVRational calc_par(int width, int height, int d_width, int d_height)
{
                  ...
                  unsigned int fmt)
{
    ...
    destroy_graph(vf);
    mp_msg(MSGT_VFILTER, MSGL_FATAL, "Create graph: '%s'\n", p->cfg_graph);
    ...
    AVRational par = calc_par(width, height, d_width, d_height);
    ...
    p->par_out = l_out->sample_aspect_ratio;

    int w = l_out->w;
    int h = l_out->h;
    int dw = w;
    int dh = h;
    int outfmt = pixfmt2imgfmt(l_out->format);

    talloc_free(tmp);
    return vf_next_config(vf, w, h, dw, dh, flags, outfmt);
